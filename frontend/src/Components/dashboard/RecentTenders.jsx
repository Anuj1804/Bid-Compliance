  ...
  return (
    <div className="w-full max-w-[140px]">
      <div className="flex items-baseline justify-between">
        <span className={`text-sm font-semibold tabular-nums ${tone.text}`}>{animatedValue.toFixed(1)}%</span>
      </div>
...
const TenderRow = ({ tender, isVisible, index }) => {
  ...
  return (
    <div
      className={`group border-b border-slate-100 px-4 sm:px-5 py-4 transition-all duration-500 ease-out last:border-b-0 hover:bg-slate-50 md:hover:translate-x-[2px] ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"
        }`}
      style={{ transitionDelay: isVisible ? delay : "0ms" }}
    >
      {/* Desktop / tablet grid row */}
      <div className="hidden lg:grid lg:grid-cols-[minmax(250px,1.6fr)_100px_150px_180px_140px] lg:items-center lg:gap-4">
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api/tenders", {
      ...
      .then(data => {
        ...
        setRecentTenders(data.slice(0, 5).map(t => ({
  id: t.id,
  tenderId: `TENDER-${String(t.id).padStart(5, "0")}`,
  title: t.title,
  bidders: t.bidder_count ?? 0,
  riskSummary: t.risk_summary || { LOW: 0, MEDIUM: 0, HIGH: 0 },
  deadline: "—",
  daysLeft: null,
  status: "ACTIVE",
})));
  ...
  return (
    <section ref={sectionRef} aria-labelledby="recent-tenders-heading" className="mb-10">
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <span
            ...
            title="Live verification results from the backend."
          >
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
        ...
        <div className="hidden lg:grid lg:grid-cols-[minmax(250px,1.6fr)_100px_150px_180px_140px] lg:items-center lg:gap-4 border-b border-slate-100 bg-slate-50/60 px-4 sm:px-5 py-2.5">
          ...
          <span className="text-[10px] font-semibold tracking-[0.1em] text-slate-400">RISK BREAKDOWN</span>
